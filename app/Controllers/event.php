...
//                          //LOCALITY / CITY EXTRACTION CODE 
                            ...
                            $dates_ttl = ( $crawler -> filter ('#container > div.page > div.page-center > div.content-center > div:nth-child(3) > div.layoutColumnsInnerRight > div.eventData > div.dates > p > strong') ) -> text ();
                                // $date_start = $this->clean_str( $dates_blk )  ;
                                      if($dates_ttl == "Upcoming dates:") {
                                        $date_blk  = $crawler -> filter ('div.dates div p:nth-child(1)') -> each ( function ($subdate) {
                                          $multi_date = $subdate-> text();
                                          return $multi_date;
                                        });
                                        $date_start = [];
                                        $date_end = [];
                                        foreach ( $date_blk as $indv_date) {
                                            $indv_date = $this->clean_str($indv_date);
                                            $date_start [] = date('Y-m-d',strtotime(str_replace('.', '-', explode (' - ', $indv_date)[0] ) ) ); 
                                            $date_end [] = date('Y-m-d',strtotime(str_replace('.', '-',explode (' - ', $indv_date)[1] ) ) );
                                          }
                                      }
                                    }
